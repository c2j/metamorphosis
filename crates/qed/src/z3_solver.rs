...
pub fn solve_equivalence(input: &QedInput) -> Result<ProofResult, ProverError> {
    ...
    if arity1 != arity2 {
        return Ok(ProofResult::NotEquivalent { counterexample: None });
    }
    if arity1 == 0 {
        return Ok(ProofResult::Equivalent);
    }
...
fn create_table_funcs(schemas: &[QedSchema]) -> HashMap<String, FuncDecl> {
    ...
    for schema in schemas {
        ...
        let func = FuncDecl::new(format!("tbl_{}", schema.name).as_str(), &refs, &Sort::bool());
...
fn compute_output_arity(
    rel: &QedRelation, schemas: &HashMap<String, QedSchema>,
) -> Result<usize, ProverError> {
    match rel {
        QedRelation::Scan { table, fields } => {
            let s = schemas.get(table)
                .ok_or_else(|| ProverError::Io(format!("unknown table: {table}")))?;
            Ok(if fields.is_empty() { s.fields.len() } else { fields.len() })
        }
...
fn encode_relation(
    rel: &QedRelation, output_vars: &[Int],
    schemas: &HashMap<String, QedSchema>, table_funcs: &HashMap<String, FuncDecl>,
    ...
    match rel {
        ...
        QedRelation::Filter { condition, input } => {
            ...
            let c = encode_expr(condition, output_vars)?
                .as_bool().ok_or_else(|| ProverError::Io("filter condition must be bool".into()))?;
            Ok(Bool::and(&[&inp, &c]))
        }
        ...
        QedRelation::QOp { input, .. } =>
            encode_relation(input, output_vars, schemas, table_funcs),
    }
}

fn encode_scan(
    table: &str, fields: &[usize], output_vars: &[Int],
    schemas: &HashMap<String, QedSchema>, table_funcs: &HashMap<String, FuncDecl>,
) -> Result<Bool, ProverError> {
    let schema = schemas.get(table)
        .ok_or_else(|| ProverError::Io(format!("unknown table: {table}")))?;
    let func = table_funcs.get(table)
    ...
    if fields.is_empty() || fields.len() == full_arity {
        ...
        let tc = func.apply(&args).as_bool()
            .ok_or_else(|| ProverError::Io("table func must return bool".into()))?;
...
fn encode_project(
    exprs: &[QedExpr], input: &QedRelation, output_vars: &[Int],
    schemas: &HashMap<String, QedSchema>, table_funcs: &HashMap<String, FuncDecl>,
) -> Result<Bool, ProverError> {
    let input_arity = compute_output_arity(input, schemas)?;
    let iv: Vec<Int> = (0..input_arity).map(|i| Int::fresh_const(&format!("pj_{i}"))).collect();
...
fn encode_join(
    left: &QedRelation, right: &QedRelation, condition: &Option<QedExpr>,
    output_vars: &[Int], schemas: &HashMap<String, QedSchema>,
    ...
    if let Some(cond) = condition {
        parts.push(encode_expr(cond, output_vars)?
            .as_bool().ok_or_else(|| ProverError::Io("join condition must be bool".into()))?);
    }
    Ok(Bool::and(&parts))
}

fn encode_values(rows: &[Vec<QedExpr>], output_vars: &[Int]) -> Result<Bool, ProverError> {
    if rows.is_empty() { return Ok(Bool::from_bool(false)); }
    let rfs: Result<Vec<Bool>, ProverError> = rows.iter().map(|row| {
        let eqs: Result<Vec<Bool>, ProverError> = row.iter().enumerate().map(|(i, expr)| {
...
fn encode_uninterpreted(name: &str, output_vars: &[Int]) -> Result<Bool, ProverError> {
    if output_vars.is_empty() { return Ok(Bool::from_bool(true)); }
...
fn encode_expr(expr: &QedExpr, vars: &[Int]) -> Result<Dynamic, ProverError> {
    match expr {
        QedExpr::ColumnRef { index } => vars.get(*index)
        ...
        QedExpr::UnOp { op, expr: inner } => {
            ...
            match op.as_str() {
                "not" => v.as_bool().map(|b| Dynamic::from(b.not()))
                    .ok_or_else(|| ProverError::Io("not: expected bool".into())),
                "neg" => {
                    let i = v.as_int().ok_or_else(|| ProverError::Io("neg: expected int".into()))?;
...
fn encode_binop(op: &str, left: &Dynamic, right: &Dynamic) -> Result<Dynamic, ProverError> {
    match op {
        ...
        "and" => {
            let l = left.as_bool().ok_or_else(|| ProverError::Io("and: expected bool lhs".into()))?;
            let r = right.as_bool().ok_or_else(|| ProverError::Io("and: expected bool rhs".into()))?;
            Ok(Dynamic::from(Bool::and(&[&l, &r])))
        }
        "or" => {
            let l = left.as_bool().ok_or_else(|| ProverError::Io("or: expected bool lhs".into()))?;
            let r = right.as_bool().ok_or_else(|| ProverError::Io("or: expected bool rhs".into()))?;
...
fn encode_cmp(op: &str, left: &Dynamic, right: &Dynamic) -> Result<Dynamic, ProverError> {
    let l = left.as_int().ok_or_else(|| ProverError::Io(format!("{op}: expected int lhs")))?;
    let r = right.as_int().ok_or_else(|| ProverError::Io(format!("{op}: expected int rhs")))?;
    let b = match op {
        "gt" => l.gt(&r),
        "lt" => l.lt(&r),
        "gte" => l.ge(&r),
...
fn make_exists(vars: &[Int], body: &Bool) -> Result<Bool, ProverError> {
    if vars.is_empty() { return Ok(body.clone()); }
