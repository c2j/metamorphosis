...
impl<'de> Deserialize<'de> for ProverDataType {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Ok(match s.to_uppercase().as_str() {
            ...
            | "TIMESTAMP" | "DATE" | "TIME" => ProverDataType::Integer,
...
pub enum ProverJoinKind { Inner, Left, Right, Full, Semi, Anti }
...
#[serde(untagged)]
pub enum ProverExpr {
    Col { column: VL, #[serde(rename = "type")] ty: ProverDataType },
    Op {
        #[serde(rename = "operator")]
        op: String,
        #[serde(rename = "operand")]
...
pub enum ProverRelation {
    Singleton,
    Scan(VL),
    Filter { condition: ProverExpr, source: Box<ProverRelation> },
    Project { #[serde(rename = "target")] columns: Vec<ProverExpr>, source: Box<ProverRelation> },
    Join { condition: ProverExpr, left: Box<ProverRelation>, right: Box<ProverRelation>, kind: ProverJoinKind },
    Correlate { left: Box<ProverRelation>, right: Box<ProverRelation>, kind: ProverJoinKind },
    ...
    Values { schema: Vec<ProverDataType>, content: Vec<Vec<ProverExpr>> },
    Aggregate { columns: Vec<ProverAggCall>, source: Box<ProverRelation> },
    Group { keys: Vec<ProverExpr>, columns: Vec<ProverAggCall>, source: Box<ProverRelation> },
    Sort {
...
pub fn map_data_type(our_type: &str) -> ProverDataType {
    match our_type.to_lowercase().as_str() {
        "integer" | "int" | "bigint" | "smallint" | "tinyint"
        | "timestamp" | "date" | "time" | "timestamptz" | "timetz" => ProverDataType::Integer,
        "real" | "float" | "double" | "decimal" | "numeric" | "float4" | "float8" => ProverDataType::Real,
        "boolean" | "bool" => ProverDataType::Boolean,
        "string" | "varchar" | "char" | "text" | "nvarchar" | "nchar" | "bpchar" => ProverDataType::String,
    ...
    match rel {
        QedRelation::Scan { table, .. } => {
            let idx = schema_index.get(table).copied().expect(
                "convert_relation: table must have a corresponding entry in schema_index"
            );
...
pub fn convert_schema(schema: &QedSchema) -> ProverSchema {
    ProverSchema {
        ...
        primary: if schema.key.is_empty() { vec![] } else { vec![schema.key.clone()] },
...
mod tests {
    ...
    fn idx() -> HashMap<String, usize> { let mut m = HashMap::new(); m.insert("t".to_string(), 0); m }

    fn qed() -> QedInput {
        QedInput { schemas: vec![QedSchema { name: "R".to_string(), types: vec!["integer".to_string(), "integer".to_string()], key: vec![0], nullable: vec![false, true], guaranteed: vec![], fields: vec!["x".to_string(), "y".to_string()] }], queries: [QedRelation::Scan { table: "R".to_string(), fields: vec![] }, QedRelation::Scan { table: "R".to_string(), fields: vec![] }], help: "test".to_string() }
    }

    #[test] fn test_convert_scan() {
        let rel = convert_relation(&QedRelation::Scan { table: "t".to_string(), fields: vec![] }, &idx());
        assert_eq!(rel, ProverRelation::Scan(VL(0)));
    }

    #[test] fn test_convert_filter() {
        let rel = convert_relation(&QedRelation::Filter { condition: QedExpr::BinOp { op: "gt".to_string(), left: Box::new(QedExpr::ColumnRef { index: 0 }), right: Box::new(QedExpr::Literal { value: QedValue::Integer { value: 10 } }) }, input: Box::new(QedRelation::Scan { table: "t".to_string(), fields: vec![] }) }, &idx());
        assert_eq!(rel, ProverRelation::Filter { condition: ProverExpr::Op { op: ">".to_string(), args: vec![ProverExpr::Col { column: VL(0), ty: ProverDataType::Integer }, ProverExpr::Op { op: "10".to_string(), args: vec![], ty: ProverDataType::Integer, rel: None }], ty: ProverDataType::Boolean, rel: None }, source: Box::new(ProverRelation::Scan(VL(0))) });
    }

    #[test] fn test_convert_project() {
        let rel = convert_relation(&QedRelation::Project { exprs: vec![QedExpr::ColumnRef { index: 0 }, QedExpr::Literal { value: QedValue::Integer { value: 1 } }], input: Box::new(QedRelation::Scan { table: "t".to_string(), fields: vec![] }) }, &idx());
        assert_eq!(rel, ProverRelation::Project { columns: vec![ProverExpr::Col { column: VL(0), ty: ProverDataType::Integer }, ProverExpr::Op { op: "1".to_string(), args: vec![], ty: ProverDataType::Integer, rel: None }], source: Box::new(ProverRelation::Scan(VL(0))) });
    }

    #[test] fn test_convert_join() {
        let mut m = HashMap::new(); m.insert("a".to_string(), 0usize); m.insert("b".to_string(), 1usize);
        let rel = convert_relation(&QedRelation::Join { left: Box::new(QedRelation::Scan { table: "a".to_string(), fields: vec![] }), right: Box::new(QedRelation::Scan { table: "b".to_string(), fields: vec![] }), condition: Some(QedExpr::BinOp { op: "eq".to_string(), left: Box::new(QedExpr::ColumnRef { index: 0 }), right: Box::new(QedExpr::ColumnRef { index: 1 }) }) }, &m);
        assert_eq!(rel, ProverRelation::Join { condition: ProverExpr::Op { op: "=".to_string(), args: vec![ProverExpr::Col { column: VL(0), ty: ProverDataType::Integer }, ProverExpr::Col { column: VL(1), ty: ProverDataType::Integer }], ty: ProverDataType::Boolean, rel: None }, left: Box::new(ProverRelation::Scan(VL(0))), right: Box::new(ProverRelation::Scan(VL(1))), kind: ProverJoinKind::Inner });
    }

    #[test] fn test_convert_union() {
        let rel = convert_relation(&QedRelation::Union { left: Box::new(QedRelation::Scan { table: "t".to_string(), fields: vec![] }), right: Box::new(QedRelation::Scan { table: "t".to_string(), fields: vec![] }) }, &idx());
        assert_eq!(rel, ProverRelation::Union(vec![ProverRelation::Scan(VL(0)), ProverRelation::Scan(VL(0))]));
    }

    #[test] fn test_convert_schema_key() {
        let s = QedSchema { name: "t".to_string(), types: vec!["integer".to_string()], key: vec![0], nullable: vec![false], guaranteed: vec![], fields: vec!["a".to_string()] };
    ...
    #[test] fn test_convert_help_is_tuple() {
        assert_eq!(convert_input(&qed(), &HashMap::new()).help, ("test".to_string(), "test".to_string()));
    }

    #[test] fn test_convert_aggregate_with_group_by() {
        let rel = convert_relation(&QedRelation::Aggregate { keys: vec![0], aggs: vec![QedAggCall { func: "sum".to_string(), arg: QedAggArg::Expr(QedExpr::ColumnRef { index: 2 }), distinct: false }], input: Box::new(QedRelation::Scan { table: "t".to_string(), fields: vec![] }) }, &idx());
        assert_eq!(rel, ProverRelation::Group { keys: vec![ProverExpr::Col { column: VL(0), ty: ProverDataType::Integer }], columns: vec![ProverAggCall { op: "sum".to_string(), args: vec![ProverExpr::Col { column: VL(2), ty: ProverDataType::Integer }], distinct: false, ignore_nulls: false, ty: ProverDataType::Integer }], source: Box::new(ProverRelation::Scan(VL(0))) });
    }

    #[test] fn test_map_data_type_variants() {
        ...
        assert_eq!(map_data_type("uuid"), ProverDataType::Custom("UUID".to_string()));
        assert_eq!(map_data_type("jsonb"), ProverDataType::Custom("JSONB".to_string()));
    }

    #[test] fn test_roundtrip_simple_query() {
    ...
    #[test] fn test_prover_input_serialization_format() {
        let prover = ProverInput { schemas: vec![ProverSchema { name: "PUBLIC.EMP".to_string(), types: vec![ProverDataType::Integer, ProverDataType::String], primary: vec![vec![0]], nullabilities: vec![false, false], guaranteed: vec![], fields: vec!["EMPNO".to_string(), "ENAME".to_string()] }], queries: (ProverRelation::Scan(VL(0)), ProverRelation::Scan(VL(0))), help: ("SELECT *".to_string(), "SELECT ENAME".to_string()) };
        let json = serde_json::to_value(&prover).expect("serialize");
        assert_eq!(json["schemas"][0]["types"], serde_json::json!(["INTEGER", "STRING"]));
        assert_eq!(json["schemas"][0]["key"], serde_json::json!([[0]]));
        assert_eq!(json["schemas"][0]["nullable"], serde_json::json!([false, false]));
        assert!(json["schemas"][0].get("guaranteed").is_none());
        assert_eq!(json["queries"][0]["scan"], serde_json::json!(0));
        assert_eq!(json["help"], serde_json::json!(["SELECT *", "SELECT ENAME"]));
    }
    ...
    #[test] fn test_convert_set_operations() {
        let sc = || QedRelation::Scan { table: "t".to_string(), fields: vec![] };
        assert_eq!(convert_relation(&QedRelation::Intersect { left: Box::new(sc()), right: Box::new(sc()) }, &idx()), ProverRelation::Intersect(vec![ProverRelation::Scan(VL(0)), ProverRelation::Scan(VL(0))]));
        assert_eq!(convert_relation(&QedRelation::Except { left: Box::new(sc()), right: Box::new(sc()) }, &idx()), ProverRelation::Except(Box::new(ProverRelation::Scan(VL(0))), Box::new(ProverRelation::Scan(VL(0)))));
        assert_eq!(convert_relation(&QedRelation::Distinct { input: Box::new(sc()) }, &idx()), ProverRelation::Distinct(Box::new(ProverRelation::Scan(VL(0)))));
    }
}
